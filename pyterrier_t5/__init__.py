...
class MonoT5ReRanker(TransformerBase):
    ...
    def transform(self, run):
        ...
        for start_idx in it:
            ...
            )  # same as start_idx:start_idx+self.batch_size
            ...
            for key, enc_value in list(enc.items()):
                enc_value = enc_value[
                    :, :-1
                ]  # chop off end of sequence token-- this will be added with the prompt
                enc_value = enc_value[
                    :, :max_vlen
                ]  # truncate any tokens that will not fit once the prompt is added
                enc[key] = torch.cat(
                    [enc_value, prompts[key][: enc_value.shape[0]]], dim=1
                )  # add in the prompt to the end
